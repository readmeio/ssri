...
describe('ssri', function () {
  ...
  describe('#parse', function () {
    ...
    it('should discard invalid format entries', function () {
      ...
      expect(ssri.parse(missingDash).toString()).toBe(false);
      expect(ssri.parse(missingAlgorithm).toString()).toBe(false);
      expect(ssri.parse(missingDigest).toString()).toBe(false);
    });
    ...
    it('should discard hashes that dont abide by the spec', function () {
      ...
      expect(ssri.parse(badAlgorithm).toString()).toBe(false);
      expect(ssri.parse(badBase64).toString()).toBe(false);
      expect(ssri.parse(badOpts).toString()).toBe(false);
    });

    it('should not allow weird stuff in sri', function () {
      const badInt = 'mdc2\u0000/../../../hello_what_am_I_doing_here-Juwtg9UFssfrRfwsXu+n/Q==';

      expect(ssri.parse(badInt).toString()).toBe(false);
    });
  });
