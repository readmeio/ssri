...
tap('parses options from integrity string in strict mode', t => {
  ...
  const integrity = `sha512-${sha}?one?two?three`;
  t.same(
    ssri.parse(integrity, { strict: true }),
    {
      sha512: [
        {
          source: integrity,
...
tap('discards invalid format entries', t => {
  ...
  t.equal(
    ssri.parse([missingDash, missingAlgorithm, missingDigest, valid].join(' ')).toString(),
    valid,
...
tap('supports strict spec parsing', t => {
  ...
  t.same(
    ssri
      .parse([badAlgorithm, badBase64, badOpts, valid].join(' '), {
        strict: true,
      })
      .toString(),
    valid,
