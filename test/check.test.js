...
test('checkData', t => {
  ...
  t.same(ssri.checkData(TEST_DATA, sri), meta, 'Buffer data successfully verified');
  t.doesNotThrow(() => {
    ssri.checkData(TEST_DATA, sri, { error: true });
  }, 'error not thrown when error: true and data verifies');
  t.same(ssri.checkData(TEST_DATA, `sha512-${hash(TEST_DATA, 'sha512')}`), meta, 'Accepts string SRI');
  t.same(
  ...
  t.same(ssri.checkData(TEST_DATA.toString('utf8'), sri), meta, 'String data successfully verified');
  t.same(
    ssri.checkData(TEST_DATA, `sha512-nope sha512-${hash(TEST_DATA, 'sha512')}`),
    meta,
    'succeeds if any of the hashes under the chosen algorithm match'
  );
  t.equal(ssri.checkData('nope', sri), false, 'returns false when verification fails');
  t.throws(
    () => {
      ssri.checkData('nope', sri, { error: true });
    },
    /Integrity checksum failed/,
    'integrity error thrown when error: true with bad data'
  );
  t.throws(
    () => {
    ...
    /data size mismatch/,
    'size error thrown when error: true with bad size'
  );
  t.equal(ssri.checkData('nope', 'sha512-nope'), false, 'returns false on invalid sri hash');
  t.equal(ssri.checkData('nope', 'garbage'), false, 'returns false on garbage sri input');
  t.equal(ssri.checkData('nope', ''), false, 'returns false on empty sri input');
  t.throws(
    () => {
      ssri.checkData('nope', '', { error: true });
    },
    /No valid integrity hashes/,
    'errors on empty sri input if error: true'
  );
